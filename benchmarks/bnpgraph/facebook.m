...
%% Load the nips graph
fileID = fopen('C:/Github/sparsedense3_paperversion/facebook/facebook.txt', 'r');
formatSpec =  '%d\t%d';
tblSize = [2 Inf];
...
%% Write to tsv
[rid, cid] = find(Gsamp);
fileID = fopen('../../data/bnpgraph_facebook.tsv','w');
formatSpec = '%d\t%d\n';
for i = 1:length(rid)
...
for i = 1:25
    ...
    [rid, cid] = find(Gsamp);
    fileID = fopen(strcat('./bnpgraph_runs/facebook', '_', int2str(i), '.tsv'),'w');
    formatSpec = '%d\t%d\n';
    for i = 1:length(rid)
        fprintf(fileID, formatSpec, int32(rid(i)), int32(cid(i)));
    end
