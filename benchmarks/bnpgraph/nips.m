...
%% Load the nips graph
fileID = fopen('../../notebooks/example/nips.tsv', 'r');
formatSpec =  '%f\t%f';
tblSize = [2 Inf];
...
niter = 100000;  nburn = 10000; nadapt = niter/4;  thin = 20; nchains = 1;
...
for i = 1:25
    ...
    [rid, cid] = find(Gsamp);
    fileID = fopen(strcat('../../notebooks/example/bnpgraph_runs/nips', '_', int2str(i), '.tsv'),'w');
    formatSpec = '%d\t%d\n';
    for i = 1:length(rid)
        fprintf(fileID, formatSpec, int32(rid(i)), int32(cid(i)));
    end
